...
async def check_sub_call(call: types.CallbackQuery, state: FSMContext):
    ...
    channel = await text_manager.get_message(f"channel")
    invite_link = await text_manager.get_message(f"invite_link")
    ...
    if not is_subscription:
        no_subscription_text = await text_manager.get_message(f"no_subscription_{target}")

        link_btn = await text_manager.get_button_text('link')
        check_btn = await text_manager.get_button_text('check_subs')
        back_btn = await text_manager.get_button_text('back')
